...
data_path = "/Users/lokee/Desktop/new copy/RE copy/origin_data/"
# files to export data
export_path = "/Users/lokee/Desktop/new copy/RE copy/data/"
...
def init_POS():
    # reading POS embedding data...
    global pos2id, word_size
    print('reading POS data...')
    f = open(data_path + 'vec.txt', "r")
    g = open(data_path + "pos2id.txt","r")
    total, size = f.readline().strip().split()[:2]
    total = (int)(total)
    word_size = (int)(size)
    pos2id = {}
    ...
    for i in range(total):
        content = f.readline().strip().split()
        tag = sent_tokenize(content[0])
        tag = nltk.pos_tag(tag)
        tagging = pos2id[tag[0][1]]
...
def init_train_files(name):
    ...
    sen_pos3 = np.zeros((total, fixlen), dtype = np.int32)
    ...
    for s in range(total):
        ...
        words, tags = map(list,zip(*tagged))
        en1_id = content[0]
        en2_id = content[1]
        en1_name = content[2]
        en2_name = content[3]
        ...
        for i, tag in enumerate(tags):
            if i >= fixlen:
                break
            elif not tag in pos2id:
                sen_pos3[s][i] = pos2id['UNK']
            else:
                sen_pos3[s][i]  = pos2id[tag]
        ...
        if (s+1) % 100 == 0:
            print(s)
    return np.array(instance_triple), np.array(instance_scope), sen_len, sen_label, sen_word, sen_pos1, sen_pos2, sen_pos3,sen_mask
...
def init_test_files(name):
    ...
    for s in range(total):
        ...
        ss.append((en1_id + '\t' + en2_id + '\t' + rel_name, sentence, tags,en1_id, en2_id, en1_name, en2_name,rel_name))
    
    ss = sorted(ss, key=lambda x:x[0])
    for s in range(total):
        unique_id, sentence,tags ,en1_id, en2_id, en1_name, en2_name, rel_name = ss[s]
        if rel_name in relation2id:
            relation = relation2id[rel_name]
        else:
            relation = relation2id['NA']
        ...
        if (s+1) % 100 == 0:
            print(s)
    return np.array(instance_entity), np.array(instance_entity_no_bag), np.array(instance_triple), np.array(instance_scope), sen_len, sen_label, sen_word, sen_pos1, sen_pos2,sen_pos3, sen_mask
...
np.save(export_path+'vec1', pos_vec)
f = open(export_path+'config', "w")
f.write(json.dumps({"word2id":word2id,"pos2id":pos2id,"relation2id":relation2id,"word_size":word_size, "fixlen":fixlen, "maxlen":maxlen}))
f.close()

sort_files("train")
sort_files("test")
instance_triple, instance_scope, train_len, train_label, train_word, train_pos1, train_pos2,train_pos3,train_mask = init_train_files("train_sort")
...
np.save(export_path+'train_pos3', train_pos3)
np.save(export_path+'train_mask', train_mask)

instance_entity, instance_entity_no_bag, instance_triple, instance_scope, test_len, test_label, test_word, test_pos1, test_pos2,test_pos3, test_mask = init_test_files("test_sort")
...
np.save(export_path+'test_pos3', test_pos3)
