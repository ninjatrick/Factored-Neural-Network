...
class Embedding(object):
    ...
    def __init__(self, is_training, word_vec,pos_vec, word, pos1, pos2,pos3):
        ...
        temp_pos_embedding = tf.get_variable(initializer=pos_vec, name = 'temp_pos_embedding', dtype=tf.float32)
        unk_pos_embedding = tf.get_variable('UNK_embedding', [FLAGS.word_size], dtype=tf.float32, initializer=tf.contrib.layers.xavier_initializer())
        self.pos_vec = tf.concat([temp_pos_embedding,
                               tf.reshape(unk_pos_embedding, [1, FLAGS.word_size]),
                               tf.reshape(tf.constant(np.zeros([FLAGS.word_size], dtype=np.float32)), [1, FLAGS.word_size])], 0)
        self.word = word
        self.pos1 = pos1
        self.pos2 = pos2
    ...
    def tag_embedding(self, var_scope = None, reuse = False):
        with tf.variable_scope(var_scope or 'tag_embedding', reuse = reuse):
            x = tf.nn.embedding_lookup(self.pos_vec, self.word)
            return x
    ...
    def concat_embedding(self, word_embedding, pos_embedding, tag_embedding):
        if pos_embedding is None:
            x = tf.concat(values = [tag_embedding,word_embedding], axis = 2)
            return x
        else:
            x = tf.concat(values = [tag_embedding,word_embedding], axis = 2)
            return tf.concat(values = [x, pos_embedding], axis = 2)
