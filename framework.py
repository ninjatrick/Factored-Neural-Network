...
class Framework(object):
    ...
    def __init__(self, is_training, use_bag=True):
        ...
        self.pos3 = tf.placeholder(dtype=tf.int32, shape=[None, FLAGS.max_length], name='input_pos3')
        ...
        self.data_pos_vec = np.load(os.path.join(FLAGS.export_path, 'vec1.npy'))

        # Network
        self.embedding = Embedding(is_training, self.data_word_vec,self.data_pos_vec, self.word, self.pos1, self.pos2,self.pos3)
    ...
    def load_train_data(self):
        ...
        self.data_train_pos3 = np.load(os.path.join(FLAGS.export_path, 'train_pos3.npy'))
        ...
        for i in self.reltot:
            self.reltot[i] = 1 / (self.reltot[i] ** (0.0001))
    ...
    def load_test_data(self):
        ...
        self.data_test_pos3 = np.load(os.path.join(FLAGS.export_path, 'test_pos3.npy'))
    ...
    def train_one_step(self, index, scope, weights, label, result_needed=[]):
        feed_dict = {
            ...
            self.pos3: self.data_train_pos3[index, :],
    ...
    def test_one_step(self, index, scope, label, result_needed=[]):
        feed_dict = {
            ...
            self.pos3: self.data_test_pos2[index, :],
    ...
    def train(self, one_step=train_one_step):
        ...
        for epoch in range(FLAGS.max_epoch):
            ...
            for i in range(int(len(train_order) / float(FLAGS.batch_size))):
                ...
                sys.stdout.write("epoch %d step %d "'\n''\n'"| loss : %f, NA accuracy: %f%%, not NA accuracy: %f%%, total accuracy %f%%" % (epoch, i, loss[0], 100 * self.acc_NA.get(), 100 * self.acc_not_NA.get(),100 * self.acc_total.get()) + '\n''\n')
                sys.stdout.flush()
